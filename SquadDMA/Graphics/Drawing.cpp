...
ID2D1Bitmap* LoadBitmapFromFile(ID2D1RenderTarget* renderTarget, IWICImagingFactory* wicFactory, const std::wstring& filePath)
{
	...
	if (SUCCEEDED(hr))
	{
		// Конвертируем изображение в формат, подходящий для Direct2D
	...
	if (SUCCEEDED(hr))
	{
		// Создаем Bitmap из конвертированного изображения
...
void DrawImageFromFile(int x, int y)
{
	...
	if (!isLoaded)
	{
		...
		if (SUCCEEDED(hr))
		{
			bitmap = LoadBitmapFromFile(RenderTarget, wicFactory, L"1.png");
			wicFactory->Release();
			isLoaded = true;
		}
	}
